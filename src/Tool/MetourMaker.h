...
void MeteorShowerMaker(vec3 anchorPoint, vec2 rangeXY, int meteourNum, vec3 direction, bool randomVelocity = false, bool randomScale = false)
{
    /*
    ...
    */
    ...
    for(int i = 0; i < meteourNum; i++)
    {
        ...
        pos.y = anchorPoint.y + GameManager::genRandFloat() * rangeXY.y;
        pos.z = 0.0f;
        ...
        if(randomScale)
        {
            ...
            float noise = GameManager::genRandFloat() * noiseScale;
            scale += noise;
        }
